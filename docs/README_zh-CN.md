...
> 💡 **自动检测原理**：开启后，AirText 会同时启动 5 种语言的并行语音识别器。在前 1~3 秒内，通过分析转录质量并结合 Apple NLLanguageRecognizer 选出最佳匹配，然后无缝切换到检测到的语言继续识别。
